...
model = SAC.load("./checkpoints/sac_so100_50000_steps", env=demo_env, device=device)

# STEP 4: Load the normalization statistics
demo_env = VecNormalize.load("./checkpoints/vec_normalize_stats_50000.pkl", demo_env)
...
if frames:
    ...
    imageio.mimsave("so100_sac_demo_50k_n.mp4", np.stack(frames), fps=25)
    print("Saved video to so100_sac_demo_50k_n.mp4")
else:
